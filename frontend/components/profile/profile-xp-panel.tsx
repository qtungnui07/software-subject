...
export const ProfileXpPanel = () => {
  ...
  return (
    <article className="rounded-[28px] border-2 border-sky-100 bg-gradient-to-br from-white via-sky-50/70 to-cyan-50 p-5 shadow-sm sm:p-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.18em] text-sky-500">
            XP Profile
          </p>
          <h2 className="mt-2 text-2xl font-black text-slate-800">Level {summary.level}</h2>
        ...
        <div className="flex size-14 shrink-0 items-center justify-center rounded-3xl border-2 border-white bg-white text-3xl shadow-sm">
          ⚡
        </div>
      </div>
      ...
      {error && (
        <div className="mt-4 rounded-2xl border-2 border-amber-100 bg-amber-50 px-4 py-3 text-sm font-bold text-amber-700">
          {error}
        </div>
      )}
      ...
      {summary.isDemoUser && (
        <p className="mt-4 rounded-2xl border-2 border-slate-100 bg-white/80 px-4 py-3 text-xs font-bold text-slate-500">

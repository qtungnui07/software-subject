  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-white px-4 py-10 text-center">
      <div className="relative mx-auto mb-6 flex size-36 items-center justify-center">
        ...
        />
        <div className={`absolute -bottom-2 -right-2 flex size-12 items-center justify-center rounded-full shadow-lg ring-4 ${badgeClasses}`}>
          {passed ? <Trophy className="size-6 stroke-[2.5]" /> : <XCircle className="size-6 stroke-[2.5]" />}
        </div>
      </div>

      <p className="mb-2 text-xs font-black uppercase tracking-[0.24em] text-sky-500">
        {lessonTitle}
      </p>
      <h1 className="max-w-[640px] text-3xl font-black leading-tight text-slate-800 md:text-4xl">
        {resultTitle}
      </h1>
      <p className="mt-3 max-w-[520px] text-base font-bold leading-relaxed text-slate-500">
        {resultDescription}
      </p>
      ...
      <div className="mt-8 grid w-full max-w-[640px] grid-cols-2 gap-4 md:grid-cols-4">
        <div className="flex flex-col items-center rounded-2xl border-2 border-green-200 bg-green-50/50 p-4 shadow-sm">
          <span className="text-xs font-black uppercase tracking-wide text-green-500">Câu đúng</span>
          <span className="mt-1 text-2xl font-black text-green-600">
            {correctAnswers}/{totalQuestions}
          </span>
        </div>

        <div className="flex flex-col items-center rounded-2xl border-2 border-rose-200 bg-rose-50/50 p-4 shadow-sm">
          <span className="text-xs font-black uppercase tracking-wide text-rose-500">Câu sai</span>
          <span className="mt-1 text-2xl font-black text-rose-600">{wrongAnswers}</span>
        </div>

        <div className="flex flex-col items-center rounded-2xl border-2 border-sky-200 bg-sky-50/50 p-4 shadow-sm">
          <span className="text-xs font-black uppercase tracking-wide text-sky-500">Độ chính xác</span>
          <span className="mt-1 text-2xl font-black text-sky-600">{accuracy}%</span>
        </div>

        <div className="flex flex-col items-center rounded-2xl border-2 border-orange-200 bg-orange-50/50 p-4 shadow-sm">
          <span className="text-xs font-black uppercase tracking-wide text-orange-500">XP</span>
      ...
      {passed && !completionBlocked ? (
        <div className="mt-4 grid w-full max-w-[420px] grid-cols-2 gap-3">
          <div className="rounded-2xl border-2 border-slate-200 bg-slate-50 px-4 py-3">
            <p className="text-xs font-black uppercase tracking-wide text-slate-400">Tổng XP</p>
            <p className="mt-1 text-lg font-black text-slate-700">{totalXp}</p>
          </div>
          <div className="rounded-2xl border-2 border-slate-200 bg-slate-50 px-4 py-3">
            <p className="text-xs font-black uppercase tracking-wide text-slate-400">Level</p>
            <p className="mt-1 text-lg font-black text-slate-700">Level {level}</p>
          </div>
        </div>
      ) : null}

      <div className="mt-4 min-h-6 max-w-[520px] px-4 text-sm font-bold text-slate-500">
        {passed
          ? completionBlocked
            ? xpStatusMessage
            : isWaitingForXp
      ...
      {alreadyClaimed && !completionBlocked ? (
        <p className="mt-2 max-w-[460px] rounded-2xl border-2 border-slate-200 bg-slate-50 px-4 py-3 text-sm font-bold text-slate-500">
          Bài học này đã nhận XP trước đó, nên hệ thống không cộng XP lần hai.
        </p>
      ) : null}

      {!passed ? (
        <p className="mt-2 max-w-[460px] rounded-2xl border-2 border-rose-200 bg-rose-50 px-4 py-3 text-sm font-bold text-rose-600">
          Kết quả hiện tại chưa mở khóa node tiếp theo. Tiến độ lộ trình được giữ nguyên.
        </p>
      ) : null}
      {completionBlocked ? (
        <p className="mt-2 max-w-[460px] rounded-2xl border-2 border-amber-200 bg-amber-50 px-4 py-3 text-sm font-bold text-amber-700">
      ...
      <div className="mt-8 flex w-full max-w-[360px] flex-col gap-3">
        ...
        <Button variant="outline" onClick={onBackToLearn} className="h-12 w-full rounded-2xl border-2 border-slate-200">

...
export const CheckpointSubmitDialog = ({
  open,
  answeredCount,
  totalQuestions,
  isSubmitting,
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/60 p-3 backdrop-blur-sm sm:items-center"
      role="dialog"
      aria-modal="true"
      aria-labelledby="checkpoint-submit-title"
    >
      <div className="w-full max-w-md rounded-3xl border-2 border-slate-200 bg-white p-5 shadow-2xl dark:border-[#2a3c44] dark:bg-[#152126] sm:p-6">
        <div className="mb-5 flex items-start justify-between gap-4">
          <div className="flex gap-3">
            ...
            <div>
              <h2 id="checkpoint-submit-title" className="text-xl font-black text-slate-900 dark:text-white">
              ...
              <p className="mt-1 text-sm font-bold leading-relaxed text-slate-500 dark:text-slate-400">
                ...
                {missingCount
                  ? ` ${missingCount} câu bỏ trống sẽ được tính 0 điểm.`
                  : " Sau khi nộp, bạn không thể sửa đáp án trong lượt này."}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <Button
            ...
            onClick={missingCount ? onReturnToMissing : onClose}
            disabled={isSubmitting}
            className="h-12 rounded-2xl"
          >
            {missingCount ? "Quay lại làm tiếp" : "Kiểm tra lại"}
          </Button>
          <Button
            type="button"
            variant="primary"

...
const getNode = (nodeId: string) => {
  const node = getCourseNodes("english").find((item) => item.id === nodeId);
  assert.ok(node, `Missing node ${nodeId}.`);

  return node;
};

const assertFreezeChest = (
  ...
  assert.equal(
    ...
    false,
    `${node.id} must not count toward section progress.`
  );
...
const allNodes = getCourseNodes("english");
...
assert.deepEqual(
  chestNodes.map((node) => node.id),
  expectedChestIds,
  "English course must contain one Freeze chest in each section."
);
...
assert.equal(
  getNode("lesson-4").unlockAfterId,
  "lesson-3",
  "Section 1 lesson 4 must not be blocked by the optional chest."
);
assert.equal(
  getNode("en-s2-c1-lesson-4").unlockAfterId,
  "en-s2-c1-lesson-3",
  "Section 2 lesson 4 must not be blocked by the optional chest."
);
assert.equal(
  getNode("en-s3-c1-lesson-4").unlockAfterId,
  "en-s3-c1-lesson-3",
  "Section 3 lesson 4 must not be blocked by the optional chest."
);

console.log(
  "Course reward contract check passed: 3 optional Freeze chests, +1 Freeze reward metadata, no progress inflation, and no lesson blocking."
);

...
const migrated = normalizeCourseProgressState(migrateChapterOneProgressToCourseProgress(legacy, englishCourse, existing), englishCourse);
...
const projected = projectCourseProgressToChapterOne(migrated, englishCourse);
...
console.log("Legacy user migration check passed: Chapter 1 progress merges without data loss, onboarding stays completed, reverse projection remains valid, and dirty IDs normalize safely.");

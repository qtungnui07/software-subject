...
const placementResultSource = read("app/placement-test/result/placement-result-client.tsx");
const progressServiceSource = read("services/course-progress-service.ts");
const packageSource = read("package.json");
assert.equal(existsSync(resolve("components/learn/section-switcher.tsx")), false, "The legacy SectionSwitcher must be removed after /sections becomes the only picker.");
assert.equal(existsSync(resolve("components/learn/locked-section-panel.tsx")), false, "The dead locked-section branch must be removed from /learn.");
...
assert.doesNotMatch(learnClientSource, /\/api\/progress\/course\/select-section/);
assert.match(learnClientSource, /section\.id === courseProgress\.currentSectionId/, "/learn must render only the persisted current section.");
...
assert(headerSource.indexOf("</Link>") < headerSource.indexOf("<GuideDialog"), "GuideDialog must remain outside the /sections Link click area.");
...
assert.match(lessonPlayerSource, /const redirectToLearn = \(\) => \{[\s\S]*?router\.push\("\/learn"\);/);
...
assert.match(progressServiceSource, /row\.currentSectionId !== existingState\.currentSectionId/);
assert.match(progressServiceSource, /persistCourseProgress\(userId, existingState\)/);
...
assert.equal(repaired.currentSectionId, "english-section-2", "Invalid stored section IDs must recover to the highest unlocked section.");
assert.match(packageSource, /"check:learn-sections-integration": "tsx scripts\/check-learn-sections-integration\.ts"/);
console.log("Learn sections integration check passed: /learn renders only currentSectionId, the blue header opens /sections without capturing Guide, legacy section queries become advisory highlights, and invalid stored sections repair safely.");

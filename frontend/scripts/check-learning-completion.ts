...
const lessonFail = getLearningCompletionPolicy("lesson-1", 59);
const lessonPass = getLearningCompletionPolicy("lesson-1", 60);
const sectionTwoPass = getLearningCompletionPolicy("en-s2-c1-lesson-1", 80);
const sectionThreePass = getLearningCompletionPolicy("en-s3-c1-lesson-1", 90);
const checkpointFail = getLearningCompletionPolicy("chapter-1-test", 69);
const checkpointPass = getLearningCompletionPolicy("chapter-1-test", 70);

assert(LESSON_COMPLETION_THRESHOLD === 60, "Lesson threshold must remain 60%.");
assert(
  CHECKPOINT_COMPLETION_THRESHOLD === 70,
  "Checkpoint threshold must remain 70%.",
);
...
assert(
  getLearningCompletionPolicy("chest-1", 100) === null,
  "Chest must never be completed through the learning completion flow.",
);
...
console.log(
  "Learning completion check passed: Section 1-3 lessons, 60/70 thresholds, chest exclusion, and accuracy normalization are valid.",
);

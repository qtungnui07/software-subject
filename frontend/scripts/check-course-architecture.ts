...
import { getCourseById, getLearningNodeById, } from "@/lib/courses/course-catalog";
...
assert(englishCourse.sections.every((section) => section.chapter.order === 1), "Every MVP section must contain Chapter 1 only.");
...
assert(JSON.stringify(adaptedNodeIds) === JSON.stringify(legacyNodeIds), "Section 1 node ids or order changed during adaptation.");
assert(sectionTwo.chapter.nodes.filter((node) => node.type === "lesson").length === 5, "Section 2 must contain 5 lessons.");
assert(sectionThree.chapter.nodes.filter((node) => node.type === "lesson").length === 5, "Section 3 must contain 5 lessons.");
assert(getCourseById(englishCourse, "english-basic")?.id === "english", "Legacy english-basic course id must resolve to english.");
assert(getLearningNodeById(englishCourse, "lesson-1")?.legacyId === 1, "Legacy lesson-1 mapping is missing.");
assert(getLearningNodeById(englishCourse, "en-s3-c1-checkpoint")?.type === "checkpoint", "Section 3 checkpoint is missing.");
if (result.warnings.length > 0) {
    console.warn(result.warnings.join("\n"));
}
console.log("Course architecture check passed: 3 sections, legacy Section 1 preserved, Sections 2-3 content-ready.");

...
const newUser = createDefaultCourseProgress(englishCourse);
...
assert.equal(emptySignupProgress.completedLessons.length > 0 ||
    emptySignupProgress.claimedChests.length > 0 ||
    emptySignupProgress.completedCheckpoint, false, "The empty Chapter 1 row created during signup must not count as learning activity.");
const legacyUser = normalizeCourseProgressState(migrateChapterOneProgressToCourseProgress({
    completedLessons: ["lesson-1"],
    claimedChests: [],
    completedCheckpoint: false,
    updatedAt: "2026-07-09T03:00:00.000Z",
}, englishCourse), englishCourse);
assert.equal(legacyUser.onboardingStatus, "completed", "Legacy Chapter 1 users must not be forced into onboarding.");
assert.equal(legacyUser.onboardingCompletedAt, "2026-07-09T03:00:00.000Z");
...
console.log("Onboarding migration check passed: new users remain pending, legacy progress is exempt, and persisted states normalize safely.");

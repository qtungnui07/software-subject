...
const defaultProgress = createDefaultCourseProgress("english");
...
assert.deepEqual(
  defaultSections.map((section) => section.order),
  [1, 2, 3],
  "Sections must remain in their learning order.",
);
...
assert.match(
  defaultSections[1]?.lockReason ?? "",
  /Checkpoint|Kiểm tra/i,
  "Locked cards must explain the real checkpoint prerequisite.",
);

for (const [index, section] of englishCourse.sections.entries()) {
  const expectedRequiredNodes = section.chapter.nodes.filter(
    (node) => node.countsTowardProgress,
  ).length;
  assert.equal(
    defaultSections[index]?.totalLessons,
    expectedRequiredNodes,
    "Reward chests must not count toward section progress.",
  );
}
...
const progressedSections = buildSectionsPageViewModels(
  progressed,
  "english-section-2",
);
...
assert.equal(
  existsSync(path.join(root, "components/learn/section-switcher.tsx")),
  false,
  "The legacy switcher must stay removed after /sections becomes the only section picker.",
);
...
console.log(
  "Sections page check passed: all three sections render in order, required-node progress excludes reward chests, locked sections explain prerequisites, requested cards highlight safely, and server selection returns learners to /learn.",
);

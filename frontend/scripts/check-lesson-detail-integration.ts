...
import { createDefaultCourseProgress, normalizeCourseProgressState, } from "@/lib/courses/course-progress";
...
assert.equal(existsSync(resolve("components/chapter-one-lesson-detail-client.tsx")), false, "The legacy Section 1 detail component must be removed after the shared page is active.");
assert.equal(existsSync(resolve("components/course-lesson-detail-bridge.tsx")), false, "The temporary Section 2-3 bridge must be removed after the shared page is active.");
const learningNodes = englishCourse.sections.flatMap((section) => section.chapter.nodes.filter((node) => node.type !== "chest"));
const detailIds = Object.keys(englishLessonDetailCatalog).sort();
const learningNodeIds = learningNodes.map((node) => node.id).sort();
assert.deepEqual(detailIds, learningNodeIds, "Every lesson/checkpoint, and only those nodes, must have authored detail content.");
const getRemoteContent = (nodeId: string) => {
    const node = learningNodes.find((candidate) => candidate.id === nodeId);
    const detail = englishLessonDetailCatalog[nodeId];
    const exercises = getExercisesForLesson(nodeId);
    assert(node);
    assert(detail);
    return {
        title: node.title,
        ...
        exerciseSummary: {
            ...
            types: exercises.map((exercise) => exercise.type),
            skills: exercises.map((exercise) => exercise.skill),
        },
    };
};
...
for (const node of learningNodes) {
    ...
    const view = buildLessonDetailViewModel(node.id, allSectionsProgress, englishCourse, getRemoteContent(node.id));
...
const completedView = buildLessonDetailViewModel(firstLessonId, completedProgress, englishCourse, getRemoteContent(firstLessonId));
assert.equal(completedView?.status, "completed");
assert.equal(completedView?.actionLabel, "Ôn tập lại");
const lockedView = buildLessonDetailViewModel("en-s3-c1-lesson-1", createDefaultCourseProgress(englishCourse), englishCourse, getRemoteContent("en-s3-c1-lesson-1"));
...
const checkpointView = buildLessonDetailViewModel("chapter-1-test", checkpointProgress, englishCourse, getRemoteContent("chapter-1-test"));
...
assert(pageSource.includes("buildLessonDetailViewModel") &&
    pageSource.includes("<CourseLessonDetail") &&
    !pageSource.includes("ChapterOneLessonDetailClient") &&
    !pageSource.includes("CourseLessonDetailBridge"), "All three sections must render through the same lesson-detail component.");
assert(pageSource.includes('node.type === "chest"') && pageSource.includes("notFound()"), "Unknown nodes and reward chests must not have lesson-detail pages.");
assert(!viewModelSource.includes("getExercisesForLesson") &&
    viewModelSource.includes("remoteContent.exerciseSummary") &&
    viewModelSource.includes("exerciseCount"), "Exercise counts and types must come from the real exercise catalog.");
assert(componentSource.includes("Mục tiêu bài học") &&
    componentSource.includes("Kỹ năng trọng tâm") &&
    componentSource.includes("Dạng bài sẽ gặp") &&
    componentSource.includes("Thông tin checkpoint"), "The shared page must expose objectives, real lesson metadata, and checkpoint information.");
assert(componentSource.includes("detail.playerHref") &&
    componentSource.includes("<Button disabled") &&
    componentSource.includes("detail.lockedReason"), "Locked lessons must remain visible but must not start the player.");
assert(packageSource.includes('"check:lesson-detail-integration": "tsx scripts/check-lesson-detail-integration.ts"'), "package.json must expose check:lesson-detail-integration.");
console.log("Lesson detail integration check passed: all 19 lessons/checkpoints share one detail page, authored objectives pair with real exercise metadata, locked content cannot start, and checkpoints show real score thresholds.");

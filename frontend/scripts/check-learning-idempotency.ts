import {
  ...
  runIdempotentLearningCompletion,
} from "../lib/learning/completion-idempotency";
...
const main = async () => {
  ...
  assert(
    !third.progress.completedNodeIds.includes("tampered"),
    "Cached results must be cloned before returning.",
  );

  console.log(
    "Learning idempotency check passed: concurrent and later retries reuse one completion result without shared mutation.",
  );
};

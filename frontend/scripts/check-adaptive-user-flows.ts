...
import { applyPlacementUnlock, canAccessCourseNode, isSectionUnlocked, } from "@/lib/courses/course-unlock-policy";
import { completeBasicOnboarding, completePlacementOnboarding, createPendingOnboardingState, startPlacementOnboarding, } from "@/lib/onboarding/onboarding-policy";
...
import { ENGLISH_PLACEMENT_TEST_VERSION, type PlacementTestSubmission, } from "@/types/placement-test";
...
const getWrongAnswer = (exercise: CoreExercise): CoreExerciseAnswer => {
    switch (exercise.type) {
        ...
        case "listening_choice":
            return {
                type: "choice",
                optionId: exercise.options.find((option) => option.id !== exercise.correctOptionId)?.id ?? exercise.correctOptionId,
            };
        case "arrange_words":
            return {
                type: "arrange_words",
...
const basicOnboarding = completeBasicOnboarding(createPendingOnboardingState(), completedAt);
const basicProgress = {
...
const sectionTwoScore = gradePlacementSubmission(buildSubmission([1, 2, 5, 6, 9]));
assert.equal(sectionTwoScore.assignedSectionId, "english-section-2");
const sectionTwoProgress = applyPlacementUnlock(createDefaultCourseProgress(englishCourse), sectionTwoScore.assignedSectionId, englishCourse);
const sectionTwoOnboarding = completePlacementOnboarding(startPlacementOnboarding(createPendingOnboardingState()), completedAt);
...
assert.equal(canAccessCourseNode(sectionTwoProgress, "en-s2-c1-lesson-1", englishCourse), true);
assert.equal(canAccessCourseNode(sectionTwoProgress, "en-s3-c1-lesson-1", englishCourse), false);
const sectionThreeScore = gradePlacementSubmission(buildSubmission([1, 2, 3, 5, 6, 7, 9, 10, 11]));
assert.equal(sectionThreeScore.assignedSectionId, "english-section-3");
const sectionThreeProgress = applyPlacementUnlock(sectionTwoProgress, sectionThreeScore.assignedSectionId, englishCourse);
assert.deepEqual(sectionThreeProgress.unlockedSectionIds, [
    "english-section-1",
    "english-section-2",
    "english-section-3",
]);
const lowerRetakeProgress = applyPlacementUnlock(sectionThreeProgress, "english-section-1", englishCourse);
assert.equal(isSectionUnlocked(lowerRetakeProgress, "english-section-3"), true, "A lower placement retake must never relock an opened section.");
console.log("Adaptive user flow check passed: Basic, Placement Section 2/3, access gates, onboarding completion, and lower-retake preservation are valid.");
